  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className={`text-[13px] ${isDark ? 'text-gray-700' : 'text-gray-500'}`}>JazzCash / EasyPaisa se rent do — automatic receipt milegi</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        ...
        <Card>
          ...
          <div className={`${isDark ? 'bg-[#1f1f1f] border-white/10' : 'bg-gray-50'} border rounded-lg p-4 mb-4`}>
            <div className={`text-[10px] font-mono mb-1 tracking-widest ${isDark ? 'text-gray-700' : 'text-gray-600'}`}>CURRENT MONTH DUE</div>
            <div className="text-2xl font-extrabold text-green-600 mb-1">Rs 9,500</div>
            <div className={`text-[12px] ${isDark ? 'text-gray-700' : 'text-gray-600'}`}>Johar Town Room · Due: 5 Feb 2025</div>
          </div>
          ...
          <div className={`text-[11px] text-center ${isDark ? 'text-gray-700' : 'text-gray-600'}`}>Auto receipt generate hogi aur save hogi</div>
        </Card>
            ...
            {[
              ...
              <div key={idx} className={`flex justify-between items-start p-3 ${isDark ? 'bg-[#1f1f1f] border-white/10' : 'bg-gray-50'} rounded-lg border`}>
                <div>
                  ...
                  <div className={`text-[11px] ${isDark ? 'text-gray-700' : 'text-gray-600'}`}>{payment.method} · {payment.date}</div>
                </div>
