...
let AuthService = (() => {
    ...
    var AuthService = _classThis = class {
        ...
        async register(dto) {
            ...
            if (existingEmail) {
                throw new common_1.ConflictException('Yeh email pehle se registered hai');
            }
            const existingPhone = await this.prisma.user.findUnique({
                where: { phone: dto.phone },
            });
            if (existingPhone) {
                throw new common_1.ConflictException('Yeh phone number pehle se registered hai');
            }
            const existingCnic = await this.prisma.user.findUnique({
                where: { cnic: dto.cnic },
            });
            if (existingCnic) {
                throw new common_1.ConflictException('Yeh CNIC pehle se registered hai');
            }
        ...
        async login(dto) {
            ...
            if (user.isSuspended) {
                throw new common_1.UnauthorizedException('Aapka account suspend kar diya gaya hai');
            }
